...
This repository also contains an [example-site](https://github.com/konstantinmuenster/portfolio-content/tree/main/example-site) that shows how the theme integrates into a Gatsby site.

---

## Table of Contents

-   [Gatsby Theme Portfolio Minimal](#portfolio-content) - [A modern one-page portfolio with a clean yet expressive design.](#a-modern-one-page-portfolio-with-a-clean-yet-expressive-design)
...
3. Now that we have our basic page layout set up, we can start adding sections to the page. It basically works the same as before: You import components from the `portfolio-content` package which then can be configured and used in your page.

    ```js
    import { AboutSection, Page, Seo } from 'portfolio-content';
...
When adding the `portfolio-content` package to the array of plugins, you can specify numerous options to adjust the functionality to your needs.
...
```jsx
import { HeroSection, Page, Section, Animation } from 'portfolio-content';
...
1. Create a file called `theme.css` in the following path: `{GatsbyProjectRoot}/src/portfolio-content/globalStyles`.
